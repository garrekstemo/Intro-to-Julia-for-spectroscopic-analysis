...
```julia
julia> [1, 2, 3]  # one-dimensional array (i.e. vector) of integers
...
If the elements are of different types, Julia will use the most general type that can hold all the elements. In this example below, the array contains objects of several data types, including strings, a float, an int, and a bool.
The vector will thus be of type `Any`.
```julia
...
And we can find out if an element is in an array using the `in` function.
```julia
julia> "hello" in a
true
```
...
We can remove elements from an array using the `pop!` and `popat!` functions.
```julia
julia> v = [1, 2, 3, 4, 5]
3-element Vector{Int64}:
 1
