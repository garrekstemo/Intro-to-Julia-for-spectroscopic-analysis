...
```julia
julia> w = [1, 2.2, 3//5]  # array of floats
...
The type `Vector` is an alias for a one-dimensional `Array` in Julia.


## Creating arrays
You can create arrays in other ways.
For example, an array of random numbers is very useful.
...
3. Look up how to create an array of zeros. Create an array of zeros with the same size as `v` with Integer type.

## Iterating over arrays
The most common ways to iterate over an array are the following.
